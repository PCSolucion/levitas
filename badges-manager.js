    ...
    renderBadges(stats) {
        ...
        sortedBadges.forEach((badge, index) => {
            ...
            if (!isUnlocked && badge.target && badge.getValue) {
                ...
                if (badge.cat === 'health') pct = 0; 
                
                const cappedPct = Math.min(100, Math.max(0, pct));
                const progressText = ['total', 'fast', 'streak', 'cons', 'water'].includes(badge.cat) ? 
            ...
            div.innerHTML = `
                ...
                <div class="w-20 h-20 rounded-full ${isUnlocked ? 'bg-gradient-to-br from-primary to-[#a78bfa]' : 'bg-white/5'} flex items-center justify-center mb-5 relative z-10 transition-transform duration-500 group-hover:scale-110" ${isUnlocked ? 'style="box-shadow: 0 10px 30px rgba(124,58,237,0.4);"' : ''}>
                    <span class="material-symbols-outlined ${isUnlocked ? 'text-white' : 'text-slate-600'} text-4xl">${badge.icon}</span>
                ...
                <div class="mt-2 w-full z-10">${progressHtml}</div>
            `;
